...
3. **堆定时器**：实现线程安全的 HeapTimer（mutex 保护），O(log n) 超时管理，自动踢掉空闲连接；
4. **异步日志系统**：有界阻塞队列 + 后台写盘线程，四级日志，按日期自动分割；
5. **生产级加固**：SIGPIPE/SIGTERM/SIGINT 信号处理、请求体大小限制、优雅关闭；
6. **压力测试**：wrk 多级并发压测（10~1000 连接），在测试中发现并修复了 6 个并发 Bug；
7. **工程化**：`.env` 环境变量配置、一键启动脚本 `run.sh`、完善文档。
...
### Q15：你遇到过的线上式 bug？
**答：**
在 wrk 压力测试中一共发现了 6 个并发 Bug：
1. **无限递归**：`CloseConn_` 调用 `doWork()` → 执行回调 → 再次调用 `CloseConn_`。修复：新增 `cancel()` 方法，只删除不执行回调；
2. **双重删除**：`tick()` 先执行回调再 `pop()`，回调中 `cancel()` 操作已被处理的节点。修复：先 `del_(0)` 删节点，再执行回调；
3. **assert 崩溃**：高并发下 `assert(users_.count(fd)>0)` 和 `assert(!heap_.empty())` 触发。修复：替换为 `if` 检查 + `continue`；
4. **双重 close(fd)**：`HttpConn::Close()` 没有标记 `fd_ = -1`，导致同一个 fd 被关闭两次。修复：close 后立即 `fd_ = -1`；
5. **线程安全**：HeapTimer 被主线程（tick/GetNextTick）和工作线程（cancel/adjust）并发访问。修复：所有公共方法加 `std::mutex`；
6. **listen backlog 过小**：`listen(fd, 6)` 导致高并发下连接被拒。修复：改为使用系统上限（当前环境为 4096），并启用 `SO_REUSEPORT`。
...
### Q23：这个架构里最大瓶颈可能在哪里？
**答：**
已将推理路径改为 Session 池并发，当前主要瓶颈转为网络层与系统资源参数（连接队列、内核调度、压测机能力）。后续可继续做批处理、NUMA/绑核优化。
...
已经实现的：
- ✅ 异步日志系统（四级日志，按日期分割）；
- ✅ 优雅退出（SIGTERM/SIGINT 信号处理，设置 `g_shutdown` 标志位）；
- ✅ 请求体大小限制（防 OOM，默认 1MB）；
- ✅ 连接超时管理（HeapTimer 自动踢空闲连接）；
- ✅ SIGPIPE 处理（`signal(SIGPIPE, SIG_IGN)`）；
- ✅ wrk 多级压测验证。
...
下一步：
1. 推理链路耗时分位指标（P50/P99）；
2. 接口自动化测试（回归验证）；
3. 推理批处理（micro-batch）与线程亲和优化，进一步提升吞吐与尾延迟稳定性；
4. HTTP/1.1 chunked transfer 支持。
...
- 架构：`Epoll(ET+ONESHOT) + 线程池 + AI Session 池并发推理`。
